...
import { Search, Plus, ChevronDown, ChevronUp, X, Pencil, Trash2, Upload, Settings } from 'lucide-react';
...
const Accounts = ({ onViewDetails }) => {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      const data = {
        ...
        purchaseCost: Number(form.get('cost')),
  ...
  return (
    <div className="nexus-page accounts-page fade-in">
      ...
      {open && <section className="inline-add-account"><div className="inline-add-head"><div><h3>{editingAccount ? 'Edit Account' : 'Buy / Add Account'}</h3><p>{editingAccount ? 'Update account details and attached games.' : 'Record the account purchase and attach one or more games from database.'}</p></div><button type="button" onClick={resetAddForm} disabled={saving}>Close</button></div><form key={editingAccount ? editingAccount.id : 'add'} onSubmit={submit}><div className="form-group"><label className="form-label">Email</label><input className="form-input" name="email" type="email" required autoFocus defaultValue={editingAccount?.email || ''}/></div><div className="form-group"><label className="form-label">Password</label><input className="form-input" name="password" type="text" placeholder="PSN account password" defaultValue={editingAccount?.password || ''}/></div><div className="form-group"><label className="form-label">Region</label><select className="form-select" name="region" defaultValue={editingAccount?.region || 'US'}><option>US</option><option>UK</option><option>TR</option><option>JP</option></select></div><div className="form-group"><label className="form-label">Cost (TZS)</label><input className="form-input" name="cost" type="number" step="1" min="1" required defaultValue={editingAccount?.purchaseCost || ''}/></div><div className="form-group"><label className="form-label">Games on this account</label>{games.length ? <div className="game-check-list">{games.map(g=><label key={g.id} className="game-check-row"><input type="checkbox" checked={selectedGames.includes(g.id)} onChange={()=>toggleGame(g.id)}/><span>{g.name}</span></label>)}</div> : <p className="empty-line">No games yet. Type a new game name below.</p>}</div><div className="form-group"><label className="form-label">+ Add new game names</label><div className="dynamic-game-inputs">{newGameNames.map((val, i)=><div key={i} className="game-input-row"><input className="form-input" value={val} onChange={(e)=>{const next=[...newGameNames]; next[i]=e.target.value; setNewGameNames(next);}} placeholder="e.g. FIFA 26 Ultimate Edition"/>{newGameNames.length>1 ? <button type="button" className="icon-shell game-input-remove" onClick={()=>setNewGameNames(newGameNames.filter((_,j)=>j!==i))}><X size={16}/></button> : null}</div>)}<button type="button" className="game-input-add" onClick={()=>setNewGameNames([...newGameNames, ''])}><Plus size={16}/> Add game</button></div></div><div className="form-group"><label className="form-label">Notes</label><textarea className="form-textarea" name="notes" placeholder="Optional notes" defaultValue={editingAccount?.notes || ''}/></div><button type="submit" className="sheet-submit-btn" disabled={saving}>{saving ? 'Saving...' : editingAccount ? 'Save changes' : 'Add account'}</button></form></section>}
