...
const AccountDetails = ({ id, accountId, onBack }) => {
  ...
  return <div className="nexus-page details-page fade-in">
    <header className="details-top"><button className="icon-shell" onClick={onBack}><ArrowLeft size={20}/></button><div className="details-title"><h1>{account.email}</h1><p>{account.region} account</p></div><button className="icon-shell" title="Copy login" onClick={()=>navigator.clipboard?.writeText(`${account.email}\n${account.password}`)}><Copy size={19}/></button></header>
...
const SlotBlock = ({ title, slots }) => <div className="slot-block"><h4>{title}</h4>{slots.map((slot)=><div key={slot.id} className={`slot-detail ${slot.status}`}><span>{slot.type === 'normal' ? `Normal ${slot.slotNumber}` : `Reset cycle ${slot.resetCycle || 1}`}</span><b>{slot.status}</b></div>)}</div>;
